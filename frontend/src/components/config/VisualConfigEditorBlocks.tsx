...
export const ApiKeysCardEditor = memo(function ApiKeysCardEditor({
  ...
  const apiKeys = useMemo(() => value.filter((item) => item.key.trim()), [value]);

  const apiKeyInputId = useId();
  const apiKeyRemarkId = `${apiKeyInputId}-remark`;
  ...
  const handleDelete = (apiKeyId: string) => {
    onChange(apiKeys.filter((item) => item.id !== apiKeyId));
  };
  ...
  const handleSave = () => {
    ...
    const nextKeys =
      editingApiKeyId === null
        ? [...apiKeys, nextEntry]
        : apiKeys.map((item) => (item.id === editingApiKeyId ? nextEntry : item));
    ...
    <div className="form-group" style={{ marginBottom: 0 }}>
      ...
      {apiKeys.length === 0 ? (
        ...
        <div className="item-list" style={{ marginTop: 4 }}>
          {apiKeys.map((entry, index) => (
            <div key={entry.id ?? `${entry.key}-${index}`} className="item-row">
              <div className="item-meta">
                <div className="pill">#{index + 1}</div>
                <div className="item-title">{entry.remark || t('config_management.visual.api_keys.input_label')}</div>
      ...
      >
        <div className="form-group">
          ...
          <label htmlFor={apiKeyRemarkId} style={{ marginTop: 12 }}>
