...
function extractApiKeyRemark(raw: unknown): string {
  const record = asRecord(raw);
  if (!record) return '';
  const candidates = [record.remark, record.owner, record.label, record.name];
  for (const candidate of candidates) {
    if (typeof candidate === 'string') {
...
function parseApiKeys(raw: unknown): VisualApiKeyEntry[] {
  ...
  for (const item of raw) {
    ...
    if (!key) continue;
    keys.push({ id: makeClientId(), key, remark: extractApiKeyRemark(item) });
  }
  return keys;
}
...
function getNextDirtyFields(
  ...
  if (Object.prototype.hasOwnProperty.call(patch, 'apiKeys')) {
    updateDirty('apiKeys', JSON.stringify(nextValues.apiKeys) === JSON.stringify(baselineValues.apiKeys));
  }
    ...
    (currentYaml: string): string => {
      try {
        ...
        const apiKeys = values.apiKeys
          .map((item) => ({ key: item.key.trim(), remark: item.remark.trim() }))
