  ...
  if (Array.isArray(apiKeysRaw)) {
    config.apiKeys = apiKeysRaw
      .map((item) => {
        ...
        const key = String(record.key ?? record['api-key'] ?? record.apiKey ?? '').trim();
        if (!key) return null;
        const remark = String(record.remark ?? record.owner ?? '').trim();
        return remark ? { key, remark } : { key };
      })
      .filter(Boolean) as NonNullable<typeof config.apiKeys>;
  }
