...
func LoadConfigOptional(configFile string, optional bool) (*Config, error) {
	...
	// Normalize top-level client API keys.
	cfg.APIKeys = cfg.APIKeys.Normalize()
...
type legacyOpenAICompatibility struct {
	...
	APIKeys APIKeyList `yaml:"api-keys"`
}
...
func (cfg *Config) migrateLegacyOpenAICompatibilityKeys(legacy []legacyOpenAICompatibility) bool {
	...
	for _, legacyEntry := range legacy {
		...
		if mergeLegacyOpenAICompatAPIKeys(target, legacyEntry.APIKeys.Keys()) {
			changed = true
		}
	}
	return changed
