...
func (h *Handler) GetAPIKeys(c *gin.Context) {
	h.mu.Lock()
	defer h.mu.Unlock()
	c.JSON(200, gin.H{"api-keys": h.cfg.APIKeys.Normalize()})
}
...
func (h *Handler) PatchAPIKeys(c *gin.Context) {
	...
	target := h.cfg.APIKeys.Normalize()
	if body.Index != nil && body.Value != nil && *body.Index >= 0 && *body.Index < len(target) {
		target[*body.Index] = *body.Value
		h.cfg.APIKeys = target.Normalize()
		h.persistLocked(c)
		return
	}
	if body.Old != nil && body.New != nil {
		...
		target = append(target, config.APIKeyEntry{Key: newKey})
		h.cfg.APIKeys = target.Normalize()
		h.persistLocked(c)
		return
	}
...
func (h *Handler) DeleteAPIKeys(c *gin.Context) {
	...
	if val := strings.TrimSpace(c.Query("value")); val != "" {
		out := make(config.APIKeyList, 0, len(target))
		for _, v := range target {
			if strings.TrimSpace(v.Key) != val {
				out = append(out, v)
			}
		}
		h.cfg.APIKeys = out.Normalize()
		h.persistLocked(c)
		return
	}
	c.JSON(400, gin.H{"error": "missing index or value"})
